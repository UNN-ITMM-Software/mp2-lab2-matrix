...
{
  if (s > MAX_TR_MATRIX_SIZE) throw "TTriangularDynamicMatrix(size_t s) s > MAX_MATRIX_SIZE";
...
{
  TTriangularDynamicMatrix(sz) result;
  for (int i = 0; i < sz; i++)
...
inline TTriangularDynamicMatrix<T> TTriangularDynamicMatrix<T>::operator+(const TTriangularDynamicMatrix<T>& m)
{
  ...
  TTriangularDynamicMatrix<T> result(sz);
  for (int i = 0; i < sz; i++)
...
inline TTriangularDynamicMatrix<T> TTriangularDynamicMatrix<T>::operator*(const TTriangularDynamicMatrix<T>& m)
{
  ...
  TTriangularDynamicMatrix<T> result(m.sz);
  for (int i = 0; i < sz; i++)
    for (int j = 0; j < sz; j++)
    {
      if (j <= i)
      {
        ...
        for (int k = 0; k < sz; k++)
          if ((k <= i) && (k >= j))
            result[i][j] += this->pMem[i][k] * m[k][j];
      }
    }
